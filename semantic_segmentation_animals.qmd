...
The `feature_extractor()` function can be used to create a function that returns a scalar value, which quantifies how much a given input image "activates" a given filter in a layer. This is the "loss function" that will be maximized during the gradient ascent process:

```{r}
# Takes in a image tensor and the index of the filter
compute_loss <- function(image, filter_index) {
  ...
  # Return mean of the activation values for the filter
  mean(filter_activation[, 3:-3, 3:-3])
}
```

-   We cast `filter_index` to a tensor integer because we want to make sure there is a consistent behavior when running the function eager (i.e. not through `tf_function()`)
-   We avoid border artifacts by only involving the non-border pixels in the loss.
    -   We discard the first two pixels along the sides of the activation

#### Create gradient ascent function using GradientTape()

```{r}
gradient_ascent_step <-
  function(image, filter_index, learning_rate) {
    # Watch the image tensor since it's not a TF Variable
    with(tf$GradientTape() %as% tape, {
      tape$watch(image)
      # Comput the loss scalar
      loss <- compute_loss(image, filter_index)
    })
    # Comput the gradients of the loss with respect to the image
    grads <- tape$gradient(loss, image)
    # Apply the gradient normalization trick
    grads <- tf$math$l2_normalize(grads)
    # Move the image and return the updated image
    image + (learning_rate * grads)
  }
```

We need to explicitly watch the image tensor since it's not a TensorFlow Variable. Only TensorFlow Variables are automatically watched by the gradient tape.

-   At the end of the function, we're moving the image a little bit in a direction that activates the target filter more strongly. We then return the updated image so that we can run the step function in a loop.

#### Function to generate filter visualizations

Now that we have all the pieces, we can put them together in a function that takes a layer and filter index as an input and returns a tensor representing the pattern that maximizes the activation of the specified filter.

-   Using the `tf_function()`, we can speed everything up.

```{r}
c(img_width, img_height) %<-% c(200, 200)

# Speed things up with tf_function
generate_filter_pattern <- tf_function(function(filter_index) {
  ...
  learning_rate <- 10
  # Initialize image tensor with random values
  image <- tf$random$uniform(minval = 0.4,
                             maxval = 0.6,
                             shape = shape(1, img_width, img_height, 3))
  # Repeatedly update the values of the image tensor
  for (i in seq(iterations))
    image <- gradient_ascent_step(image, filter_index, learning_rate)
  
  # Drop the batch dim and return the image
  image[1, , ,]
})
```

-   Since the Xception model expects an input value in the \[0,1\] when are centering the range of the random values on 0.5

-   We need to repeatedly update the values of the image tensor to maximize the loss function
...
-   `mean()` invokes `tf$math$reduce_mean()`
...
-   Note: `filter_index` was cast with `as_tensor()` because a `tf_function()` compiles a separate optimization function for each unique way it is called and a different constant literal counts as a unique call signature.

We can now visualize every filter in a layer, or every filter in every layer for that matter.

#### Generating a grid of all filter responses in a layer

```{r}
par(mfrow = c(8, 8))
# Generate and plot visualizations for the first 64 filters in the layer
for (i in seq(0, 64)) {
...
Each layer in a convnet learns a collection of filters such that their inputs can be expressed as a combinations of the filters. The filters in these convnet filter banks get increasingly complex and refined as we go deeper into the model:

-   The filters in the first layers in the model encode simple directional edges and colors.

-   The filters a bit further up in the stack, such as `block4_sepcov1`, encode simple textures made up from combinations of edges and colors

-   The filters in the higher layers begin to resemble textures found in natural images, such as features, eyes, leaves, etc.

### Visualizing heatmaps of class activations

The class activation map (CAM) visualization consists of producing heatmaps of class activations over input images.

-   CAM is useful in understanding which parts of an image led to the convnet's final classification decision.

-   It's also helpful for debugging the decision process, especially with classification errors (called model interpretability)

A class activation heatmap is a 2D grid of scores associated with a specific output class for each location in an image, which indicates how important each location was in the class that was chosen.

-   With an input image in the dog-vs-cats convnet, the CAM visualization would create a heatmap for the class "cat", which would show the cat-likeliness of different parts of the image.

For this example, we'll use Grad-CAM, which consists of taking the output feature map of a convolution layer and weighting every channel by th egradient of the class with respect to the channel.

-   We can image that we're weighing a spatial map of how intensely the input image activates different channels by how important each channel is with regard to the class.

    -   This results in a spatial map of how intensely the input image activates the class

For this example, we'll use an image of two African elephants, one larger and one smaller.

#### Load the Xception network with pre-trained weigh

```{r}
# We'll keep the top densly connected classifiers
model <- application_xception(weights = "imagenet")
```

#### Pre-process the image for Xception

In order to get the image to work with the Xception model, we want it to be 299 x 299, convert it to a `float32` tensor, and preprocess it.

```{r}
# Dowanload image and store it in the path img_path
img_path <- get_file(fname = "elephant.jpg",
                     origin = "https://img-datasets.s3.amazonaws.com/elephant.jpg")

# Read image as a tensor with size 299 x 299 and type float32 (299, 299, 3)
img_tensor <- tf_read_image(img_path, resize = c(299, 299))
# Add dimension to turn the array into a batch of size (1, 299, 299, 3)
preprocessed_img <- img_tensor[tf$newaxis, , ,] |>
  application_preprocess_inputs(model = model)
```

We can now run the pretrained network on the image and decode its prediction vector back to something we can understand:
...
-   The top class is African elephant, with a 90% probability.

-   Even though it recognizes the animal in the image with a high probability (and is correct), it doesn't know how many elephants are in the image.

We can see the entry in the prediction vector that was maximally activated that corresponded the African elephant class

```{r}
which.max(preds[1, ])
```

To visualize which parts of the iamge are most African-elephant-like, we can use the Grad-CAM process.

First, we need to create a model that maps the input image to the activations of the last convolutional layer.

#### Create model that returns last convolutional outpout

```{r}
last_conv_layer_name <- "block14_sepconv2_act"
# Name sof the last two layers in the Xception model
...
Next, we create a model that maps the activations of the last convolutional layer to the final class predictions

#### Reapplying the classifier to the top of the last convolutional output
...
We now compute the gradient of the top predicted class for the input image with respect to the activations of the last convolution layer.

```{r}
with (tf$GradientTape() %as% tape, {
  last_conv_layer_output <- last_conv_layer_model(preprocessed_img)
  # Explicity watch the activations of the last conv layer
  tape$watch(last_conv_layer_output)
  preds <- classifier_model(last_conv_layer_output)
  top_pred_index <- tf$argmax(preds[1, ])
  # Retrieve activation channel that corresponds to the top predicted class
...
-   `grads` is the gradient of the top predicted class with regard to the output feature map of the last convolutional layer.

#### Gradient pooling and channel-importance weighting

Now we can apply pooling and importance weighting to the gradient tensor to generate the heatmap of the class activation.

```{r}
pooled_grads <- mean(grads, axis = c(1, 2, 3), keepdims = TRUE)

heatmap <-
  # Multiply each channel in the output of the conv layer by channel importance
  (last_conv_layer_output * pooled_grads) |>
  mean(axis = -1) |>
  # Drop batch dimension so that the output shape is (10, 10)
  _[1, , ]
```

-   `pooled_grads` is a vector where each entry is the mean intensity of the gradient for a given channel.

    -   It quantifies the importance of each channel with respect to the the top predicted class.

-   We can take advantage of the Tensor broadcasting rules to avoid using a for loop.

-   The size-1 axes of the `pooled_grads` are automatically broadcast to match the corresponding axis of the `last_conv_layer_output`.

-   `grads` and `last_conv_layer_output` have the same shape (1, 10, 10 ,2048)

-   `pooled_grads` has shape (1, 1, 1, 2048)

-   The channel-wise mean of the resulting feature map is the heatmap of call activation
...
#### Superimposing the heatmap over the original picture

We can now superimpose the activations heatmap over the original image.

-   We can use `cut()` on the heatmap values to a create a sequential color palette, and then convert it to an R `raster` object

-   Note: We need to make the heatmap a bit transparent in order to see the original image underneath.

    -   This is done by including the `alpha = 0.4` argument

```{r}
# Create semi-transparent color palette for heatmap
pal <- hcl.colors(256, palette = "Spectral", alpha = 0.4, rev = TRUE)
# Transform heatmap into an R array
heatmap <- as.array(heatmap)
# Cut the heatmap color values to sequential color palette and convert to raster
heatmap[] <- pal[cut(heatmap, 256)]
heatmap <- as.raster(heatmap)

# Load the original photo without resizing
...
-   The `rasterImage()` is superimposing the heatmap over the image, with 40% opacity. We pass `ncol(img)` and `nrow(img)` in order for the heatmap (which has fewer pixels), to be drawn on the full size of the original image.

-   Passing `interpolate = FALSE` was used to see where the activation map pixel boundaries are.

Using the heatmap we can answer two important questions:

-   Why did the network think the image contained African elephants?

-   Where is/are the elephants located in the image

Note: The ears of the the baby elephant are strongly activated, which is probably how the network distinguished between different elephant types.
