...
#### Load the Xception network with pre-trained weights
...
application_decode_predictions(model, preds, top = 3)[[1]]
```
...
To visualize which parts of the image are most African-elephant-like, we can use the Grad-CAM process.

First, we need to create a model that maps the input image to the activations of the last convolutional layer.

#### Create model that returns last convolutional output
