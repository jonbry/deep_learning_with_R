...
```{r}
holder <- list(nrow = length())
please_iter <- as_iterator(train_dataset)
for (i in 1:length(train_dataset)){
  element <- iter_next(please_iter)
...
get_features_and_labels <- function(model, dataset) {
  ...
  for (i in 1:2) {
    ...
    all_features <- listarrays::bind_on_rows(all_features, 1)
    all_labels <- listarrays::bind_on_rows(all_labels, 1)

    list(all_features, all_labels)
}
test <- get_features_and_labels(conv_base, train_dataset)
