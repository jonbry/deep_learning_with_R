...
get_features_and_labels <- function(model, dataset) {
  ...
  # Create lists to hold TF Datasets that have been processed
  all_features <- vector("list", n_batches)
  all_labels <- vector("list", n_batches)
  # Turn datset into an interator
  ...
  for (i in 1:n_batches) {
    ...
    # Predict the features based on the preprocessed images
    # Note: predict() returns R arrays no matter what it is given
    features <- conv_base %>% predict(preprocessed_images)
    # Add batch to labels and features list
    all_labels[[i]] <- labels
    all_features[[i]] <- features
   }
   # Create list of all features and labels
   list(all_features, all_labels)
}

c(train_features, train_labels) %<-% get_features_and_labels(conv_base, train_dataset)

c(val_features, val_labels) %<-% get_features_and_labels(conv_base, validation_dataset)

c(test_features, test_labels) %<-% get_features_and_labels(conv_base, test_dataset)

# Since *_features are R arrays, we need to convert labels to R arrays from TF Datasets. Inputs into the model have to be the same type.
# Below is the fast and dirty approach to get it to work. Can easily be a function

train_labels2 <- vector("list", length(train_labels))
for (i in seq_along(train_labels2)) {
  value <- as.array(train_labels[[i]])
  train_labels2[[i]] <- value
}

test_labels2 <- vector("list", length(test_labels))
for (i in seq_along(test_labels2)) {
  value <- as.array(test_labels[[i]])
  test_labels2[[i]] <- value
}

val_labels2 <- vector("list", length(val_labels))
for (i in seq_along(val_labels2)) {
  value <- as.array(val_labels[[i]])
  val_labels2[[i]] <- value
}

# Combine each list of lists into a single list
train_features2 <- listarrays::bind_on_rows(train_features) # I THINK THIS WORKS!!!
train_labels2 <- listarrays::bind_on_rows(train_labels2)

val_features <- listarrays::bind_on_rows(val_features)
val_labels2 <- listarrays::bind_on_rows(val_labels2)

test_features <- listarrays::bind_on_rows(test_features)
test_labels2 <- listarrays::bind_on_rows(test_labels2)
...
history <- model %>% fit(
  ...
  train_labels2,
  epochs = 20,
  validation_data = list(val_features,val_labels2),
  callbacks = callbacks
)
...
Using the VGG16 base, we reach a validation accuracy of about 97%, but we're overfitting very early even though we use dropout with a rather large rate.

-   This is due to the fact we can't use data augmentation with this technique, which is important for preventing overfitting with small image datasets.

#### Feature extraction with data augmentation

It will be much more expensive and slower to include data augmentation when extracting features during training, but we will get a higher accuracy.

-   This is done by creating a model that chains the `conv_base` with a new dense classifier, and training it end to end on the inputs.

We will first need to [freeze the convolutional base]{.underline}, which can be done by calling `freeze_weights()`:
...
By freezing the weights, we are emptying the list of trainable weights of the layer/model, which can be seen below:

```{r, eval = FALSE}
unfreeze_weights(conv_base)
cat("Number of trainable weights before freezing:",
    length(conv_base$trainable_weights), "\n")

freeze_weights(conv_base)
cat("Number of trainable weights after freezing:",
    length(conv_base$trainable_weights), "\n")
```

Now that the weights are frozen, we can create a new model that changes together:
...
-   Note: This is going to take longer to overfit due to data augmentation so this should really be done using the GPU.
...
-   The test accuracy is \~97%, which is a slight improvement, even though the model performed great on the validation data.

## Fine-tuning a pre-trained model

There are 5 steps for fine-tuning a network:

1.  Add custom network on top of a trained base network
2.  Freeze the base network
3.  Train the custom network that was added to the top
4.  Unfreeze some layers in the base network
    -   There are no batch normalization layers in VGG16 to worry about.
5.  Jointly train both the unfrozen layers and the custom network added to the top of the base network.

Since we've already completed the first 3 steps, we can skip to step 4, which is unfreezing a few of the layers on the base network.
...
-   We will fine-tune the last three convolutional layers, which means that all of the layers up to `block4_pool` will be frozen.

-   This means that layers `block5_conv1`, `block5_conv2`, and `block5_conv3` will be trainable.

#### Freeze layers of convolutional base

We'll start by freezing all of the layers, except 4 from the last layer.

```{r}
unfreeze_weights(conv_base, from = -4)
```

-   `from = -4` is shorthand for `length(conv_base$layers) + 1 - 4`

If done correctly, we should see all but the last 4 layers as non-trainable:

```{r}
conv_base
```

-   The output with `keras3` package is much better than `keras` as it shows that one can't train or not train the pooling layers.

#### Fine-tuning the model

To fine-tune the model, we'll use the RMSprop optimizer with a very low learning rate. We want the low learning rate in order to limit the magnitude of the modifications made to the representations of the three layers that are being fine-tuned.

-   Updates that are too large may harm the representations.
...
cat(sprintf("The test accuracy is: %.3f/n", result["accuracy"]))
```

-   The test accuracy with fine-tuning is \~97%
